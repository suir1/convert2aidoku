...
_PREFERENCE = re.compile(
    r"\b(?P<kind>ListPreference|SwitchPreferenceCompat|EditTextPreference)"
    r"\s*\([^)]*\)\s*\.apply\s*\{"
)
...
def _default(block: str, constants: dict[str, str]) -> str | bool | None:
    match = re.search(
...
def _item(kind: str, block: str, constants: dict[str, str]) -> dict[str, Any] | None:
    ...
    default = _default(block, constants)
        ...
        item.update(
            {
                "titles": titles,
                "values": values,
                "default": default if isinstance(default, str) and default in values else values[0],
            }
        )
...
def _kotlin_items(ir: SourceIR) -> list[dict[str, Any]]:
    ...
    for source in ir.files:
        for match in _PREFERENCE.finditer(source.content):
            ...
            if block is None:
                continue
            if item := _item(match.group("kind"), block, constants):
                items.append(item)
    return items
